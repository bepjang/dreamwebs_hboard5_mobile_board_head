...
<section id="bo_w">
    ...
    <div id="bo_w" class="tbl_frm01 tbl_wrap">
        ...
        <?php if ($is_name) { ?>
        <tr>
            <td><input type="text" name="wr_name" value="<?php echo $name ?>" id="wr_name" required class="frm_input required" maxlength="20" placeholder="이름"></td>
        </tr>
        <?php } ?>

        <?php if ($is_password) { ?>
        <tr>
            <td><input type="password" name="wr_password" id="wr_password" <?php echo $password_required ?> class="frm_input <?php echo $password_required ?>" maxlength="20" placeholder="비밀번호"></td>
        </tr>
        <?php } ?>

        <?php if ($is_email) { ?>
        <tr>
            <td><input type="email" name="wr_email" value="<?php echo $email ?>" id="wr_email" class="frm_input email" maxlength="100" placeholder="이메일"></td>
        </tr>
        <?php } ?>
...
<?php /*?>        <?php if ($option) { ?>
        ...
        <tr>
            <td><input type="text" style="height:40px;" name="wr_subject" value="<?php echo $subject ?>" id="wr_subject" required class="frm_input required" placeholder="제목"></td>
        </tr>

        <tr>
            <td class="wr_content">
		<?php if($editor_form) { ?>
            <?php echo $editor_form; ?>
		<?php } else { ?>
			<div class="wr_content_wrap" style="margin-right:4px;">
				<textarea style="height:100px;" id="wr_content" name="wr_content" rows="10" itemname="내용" required placeholder="내용"
		        <?php if ($write_min || $write_max) { ?>onkeyup="check_byte('wr_content', 'char_count');"<?php } ?>><?php echo $content; ?></textarea>
...
<?php /*?>        <?php for ($i=1; $is_link && $i<=G5_LINK_COUNT; $i++) { ?>
        ...
        <tr>
            <td>
            	<p style="color:red;font-size:13px;">파일첨부시 선택창이 안 나올경우  크롬브라우져를 이용해 주세요</p>
            </td>
        </tr>
    ...
    <div class="btn_confirm">
        <input type="submit"  style="width:97%; height:38px; font-size:15px; border-color: #CCC;" value="등록하기" id="btn_submit" class="btn_b02" accesskey="s">       
    </div>
